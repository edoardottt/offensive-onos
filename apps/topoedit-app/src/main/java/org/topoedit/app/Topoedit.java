...
public class Topoedit {
    ...
    @Reference(cardinality = ReferenceCardinality.MANDATORY)
    protected TopologyStore topologyStore;
    ...
    protected void activate() {
        coreService.registerApplication("org.edoardottt.topoedit.app", () -> log.info("Periscope down."));
        // startTimer(TIMEOUT);
        editTopoStore();
        log.info("Started topoedit App!");
    }
    ...
    protected void deactivate() {
        // timer.cancel();
        // timer.purge();
        log.info("Stopped topoedit App!");
    }

    // editHostStore mess up with the Host Data Store.
    private void editTopoStore() {
        /*
         * // --- ORIGINAL CODE ---
         * getHosts();
        ...
        long nanos = 9223372036854775807L;
        ...
        while (len > 0) {
            ...
            i = linkStore.getLinks();
            links = new ArrayList<Link>();
            i.forEach(links::add);
            len = links.size();
        }

        Iterable<Link> linkss = linkStore.getLinks();

        log.info("Links: {}", linkss);

        DefaultAnnotations sa = DefaultAnnotations.builder().build();

        DefaultGraphDescription gd = new DefaultGraphDescription(nanos, millis, devices, linkss, sa);

        List<Event> reasons = new ArrayList<>();
        topologyStore.updateTopology(ProviderId.NONE, gd, reasons);
    }
